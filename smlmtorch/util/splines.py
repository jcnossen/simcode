...
class SplineBase(torch.nn.Module):
    ...
    def _spline_idx(self, coord, axis : int):
        ...
        return idx, c @ self.spline_basis
...
class FixedSpline1D(torch.nn.Module):
    def __init__(self, coeffs):
        super(FixedSpline1D, self).__init__()
        
        self.coeffs = coeffs

    def forward(self, x):
        idx = x.to(torch.int64)
        idx = torch.clip(idx, 0, len(self.coeffs)-1)
        s = torch.clip(x-idx, 0, 1)

        c = (s[:,None]**(torch.arange(4, device=idx.device)[None]))
        return (self.coeffs[idx] * c[:,:,None]).sum(1)
        
            

class Spline1D(SplineBase):
    """
    Spline that maps a 1D coordinate onto a n-d output
    """
    def __init__(self, knots=None, catmull=True):
        super(Spline1D, self).__init__(knots, catmull=catmull)

    def fixed_spline(self, device=None):
        """
        returns a version of the spline where all the weights are fixed, for faster evaluation
        """
        idx = torch.arange(len(self.knots),device=device)-1
        idx = torch.clip((idx[:,None] + torch.arange(4, device=device)[None,:]), 0, len(self.knots)-1)
        coeff = (self.knots[idx][:,None]* self.spline_basis[None,:,:,None]).sum(2)
            
        return FixedSpline1D(coeff)
    
    def forward(self, x):
        ix, cx = self._spline_idx(x, 0)
        return (self.knots[ix] * cx[:,:,None]).sum(1)
    
    def forward_knots(self, x, knots):
        ix, cx = self._spline_idx(x, 0)
        return (knots[ix] * cx[:,:,None]).sum(1)
        
    @torch.jit.export
    def deriv(self, x):
        """
        returns value at x, but also derivatives w.r.t. xyz:
            
        tuple: 
            values: [ len(pts) ]
            derivs: [ len(pts) ]
        """
        ix, cx, deriv_cx = self._spline_idx_deriv(x, 0)
        return (self.knots[ix] * cx[:,:,None]).sum(1), (self.knots[ix] * deriv_cx[:,:,None]).sum(1)
        

class CatmullRomSpline1D(Spline1D):
    def __init__(self, knots=None, **kwargs):
        super(CatmullRomSpline1D, self).__init__(knots, catmull=True, **kwargs)



class HermiteSpline1D(Spline1D):
    def __init__(self, knots):
        super(HermiteSpline1D, self).__init__(knots, catmull=False)
        
    def _segment_integral(self, xs,xe):
        """
        Evaluate the integral between xs and xe, assuming it only spans one spline segment.
        """
        idx = xs.to(torch.int64)
        xs = xs-idx
        xe = xe-idx

        idx_ = torch.clip(( (idx*2)[:,None] + torch.arange(4)[None,:]), 0, len(self.knots)-1)

        a = torch.arange(4)[None,:]
        w = ( (xe[:,None]**(a+1)/(a+1) - xs[:,None]**(a+1)/(a+1)) @ self.spline_basis)
        #print(f"torch w={_S(w)}")
        sel_knots = self.knots[idx_]
        #print(f"torch sel_knots={sel_knots}")
        return (w[:,:,None] * sel_knots).sum(1)

    @torch.jit.export
    def unit_integral(self, x):
        """
        Return the integral over the spline from x-0.5 to x+0.5
        """
        x0 = x-0.5
        x1 = torch.ceil(x0)
        x2 = x+0.5
    
        return self._segment_integral(x0, x1) + self._segment_integral(x1, x2)



class FixedSpline2D(torch.nn.Module):
    def __init__(self, coeffs):
        super().__init__()
        
        self.coeffs = coeffs
        
    def _spline_idx(self, x, axis):
        idx = x.to(torch.int64)
    ...
    def forward(self, pts):
        ix, cx = self._spline_idx(pts[:,1], 1)
        iy, cy = self._spline_idx(pts[:,0], 0)
        sel_knots = self.coeffs[iy,ix]
        ...
        return (sel_knots * cy[:,:,None,None] * cx[:,None,:,None]).sum((1,2))
...
class Spline2D(SplineBase):
    ...
    """
    def __init__(self, knots, catmull):
        super(Spline2D, self).__init__(knots, catmull=catmull)
        
        assert(len(knots.shape)==3)
    ...
    def fixed_spline(self, device=None):
        ...
        ix = torch.arange(self.knots.shape[1]-1,device=device)-1
        ix = torch.clip((ix[:,None] + torch.arange(4, device=device)[None,:]), 0, self.knots.shape[1]-1)

        iy = torch.arange(self.knots.shape[0]-1,device=device)-1
        iy = torch.clip((iy[:,None] + torch.arange(4, device=device)[None,:]), 0, self.knots.shape[0]-1)

        coeff_y = (self.knots[iy[:,None]] * self.spline_basis[None,:,:,None,None]).sum(2)

        coeff = (coeff_y[:,:,ix][:,:,:,None] * self.spline_basis[None,None,None,:,:,None]).sum(4)
        coeff = coeff.permute((0,2,1,3,4))
                
        return FixedSpline2D(coeff)

    def _spline_idx_test(self, coord, axis):
        idx = coord.to(torch.int64)
        s = torch.clip(coord-idx, 0, 1)
    ...
    def forward(self, pts):
        ...
        return (self.knots[iy[:,:,None], ix[:,None,:]] * cy[:,:,None,None] * cx[:,None,:,None]).sum((1,2))
    ...
    def deriv(self, x):
        ...
        tuple: 
            ...
            derivs: [ len(pts) ]
        """
        iy, cy, dcy = self._spline_idx_deriv(x[:,0], 0)
        ix, cx, dcx = self._spline_idx_deriv(x[:,1], 1)

        values = (self.knots[iy[:,:,None], ix[:,None,:]] * cy[:,:,None,None] * cx[:,None,:,None]).sum((1,2))
        deriv_x = (self.knots[iy[:,:,None], ix[:,None,:]] * cy[:,:,None,None] * dcx[:,None,:,None]).sum((1,2))
        deriv_y = (self.knots[iy[:,:,None], ix[:,None,:]] * dcy[:,:,None,None] * cx[:,None,:,None]).sum((1,2))
        
        return values, torch.stack((deriv_y, deriv_x))


class CatmullRomSpline2D(Spline2D):
    def __init__(self, knots):
        super(CatmullRomSpline2D, self).__init__(knots, True)

class HermiteSpline2D(Spline2D):
    def __init__(self, knots):
        super(HermiteSpline2D, self).__init__(knots, False)
    ...
    def _segment_integral(self, xstart, xend, ystart, yend):
        ix, cx = self._segment_integral_idx(xstart, xend, 1)
        iy, cy = self._segment_integral_idx(ystart, yend, 0)
    ...
    def unit_area_integral(self, pts):
        ...
        return (
            self._segment_integral(x0, x1, y0, y1) + 
            self._segment_integral(x1, x2, y0, y1) + 
            self._segment_integral(x0, x1, y1, y2) + 
            self._segment_integral(x1, x2, y1, y2)
        )
...
class Spline3D(SplineBase):
    ...
    """
    def __init__(self, knots, catmull):
        super(Spline3D, self).__init__(knots, catmull=catmull)
...
class CatmullRomSpline3D(Spline3D):
    def __init__(self, knots):
        super(CatmullRomSpline3D, self).__init__(knots, True)
...
class FixedSpline3D(torch.nn.Module):
    ...
    def forward(self, pts):
        ...
        sel_knots = self.coeffs[iz,iy,ix]

        # Two options. Second one does 3900 frames/s, first one 3200 frames/s
        #value = (((sel_knots * cz[:,:,None,None]).sum(1) * cy[:,:,None]).sum(1) * cx).sum(1)

        value = sel_knots * cz[:,:,None,None,None] * cy[:,None,:,None,None] * cx[:,None,None,:,None]
    ...
    def deriv(self, pts):
        ...
        value = (sel_knots * cz[:,:,None,None,None] * cy[:,None,:,None,None] * cx[:,None,None,:,None]).sum((1,2,3))
        deriv_z = (sel_knots * dcz[:,:,None,None,None] * cy[:,None,:,None,None] * cx[:,None,None,:,None]).sum((1,2,3))
        deriv_y = (sel_knots * cz[:,:,None,None,None] * dcy[:,None,:,None,None] * cx[:,None,None,:,None]).sum((1,2,3))
        deriv_x = (sel_knots * cz[:,:,None,None,None] * cy[:,None,:,None,None] * dcx[:,None,None,:,None]).sum((1,2,3))
...
def example3D_catmull_integral():
    ...
    print(f"Unit area integral: Algebraic: {int_a:.4f}, Numerical: {int_n:.4f}. Center eval: {center:.3f}. Center eval (precomputed): {center_fs:.3f}")
    assert abs(int_a - int_n) < 0.01
